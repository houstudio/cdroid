...
TEST_F(CtsAnimatedStateListDrawableTest, testGetConstantState) {
    AnimatedStateListDrawable drawable;
...
TEST_F(CtsAnimatedStateListDrawableTest, testGetChangingConfigurations) {
    AnimatedStateListDrawable drawable;
    auto constantState = drawable.getConstantState();
    ASSERT_NE(nullptr, constantState);
...
TEST_F(CtsAnimatedStateListDrawableTest, testMutate) {
    // copy-on-write: sibling (from newDrawable) shares state; mutate gives a private copy.
    AnimatedStateListDrawable d1;
    AnimatedStateListDrawable* sibling = dynamic_cast<AnimatedStateListDrawable*>(d1.getConstantState()->newDrawable());
