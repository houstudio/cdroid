...
- **Resources**: `src/gui/res/` (the slim framework-res base) compiled into **`.pak`** ZIPs (`cdroid.pak` + one per app) by `scripts/pakbuilder.py` — one `PakBuilder` class does it all: binary AXML via aapt2 + `resources.arsc` + aapt-compiled 9-patch (cdNp chunk) + verbatim binaries; `R.h` comes from `scripts/aapt2_gen_rh.py` (real arsc IDs; idgen is retired). **Res overlay** (AOSP static device-overlay semantics, build-time only): `apps/<name>/overlay/` shadows the framework res for that app — values* files merge per `(type, name)` entry, everything else whole-file by relative path — producing a dedicated `cdroid.pak` beside the app binary (`App::findSharedPak` probes the binary dir first, so it wins for that app only). `CDROID_OVERLAY` (cache var) does the same for the shared `cdroid.pak`. ID-defining entries (`attr`/`public*`/`symbols`) are rejected — overlay only changes values/adds non-ID resources, so pinned framework IDs never move.
