...
class cairo_container :	public litehtml::document_container {
  public:
    ...
    typedef std::map<std::string, image_ptr>	images_map;
  ...
  public:
    ...
    virtual void make_url(const std::string&url,const std::string&basepath, std::string& out ) = 0;
    virtual image_ptr get_image(const std::string&url, bool redraw_on_ready) = 0;
    void clear_images();
    void add_image(const std::string& url, image_ptr& img);
  ...
  private:
    /*simpledib::dib* get_dib(litehtml::uint_ptr hdc)	{
        return (simpledib::dib*) hdc;
    }*/
