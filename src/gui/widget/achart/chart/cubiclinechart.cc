...
void CubicLineChart::calc(const std::vector<float>& points, PointF& result, int index1, int index2, float multiplier) {
    const float p1x = points.at(index1);
    const float p1y = points.at(index1 + 1);
    const float p2x = points.at(index2);
    const float p2y = points.at(index2 + 1);

    const float diffX = p2x - p1x; // p2.x - p1.x;
    const float diffY = p2y - p1y; // p2.y - p1.y;
