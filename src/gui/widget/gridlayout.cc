...
GridLayout::LayoutParams::LayoutParams(std::shared_ptr<Spec> rowSpec,std::shared_ptr<Spec> columnSpec):
...
void GridLayout::LayoutParams::init(Context* context,const AttributeSet& attrs){
    ...
    auto ta = context->obtainStyledAttributes(attrs, internal::R::styleable::GridLayoutLayout);
    if (ta) {

    const int gravity = ta->getInt(internal::R::styleable::GridLayoutLayout_layout_gravity, Gravity::NO_GRAVITY);

    const int column = ta->getInt(internal::R::styleable::GridLayoutLayout_layout_column, DEFAULT_COLUMN);
    const int colSpan = ta->getInt(internal::R::styleable::GridLayoutLayout_layout_columnSpan, DEFAULT_SPAN_SIZE);
    const float colWeight = ta->getFloat(internal::R::styleable::GridLayoutLayout_layout_columnWeight, Spec::DEFAULT_WEIGHT);
    this->columnSpec = spec(column, colSpan, getAlignment(gravity, true), colWeight);

    const int row = ta->getInt(internal::R::styleable::GridLayoutLayout_layout_row, DEFAULT_ROW);
    const int rowSpan = ta->getInt(internal::R::styleable::GridLayoutLayout_layout_rowSpan, DEFAULT_SPAN_SIZE);
    const float rowWeight = ta->getFloat(internal::R::styleable::GridLayoutLayout_layout_rowWeight, Spec::DEFAULT_WEIGHT);
...
GridLayout::GridLayout(Context*ctx,const AttributeSet* pAttrs,int defStyleAttr)
    ...
    initGridLayout();
    // Phase 2: TypedArray (binary AXML typed resolution). ta=null → text XML fallback.
    auto ta = ctx->obtainStyledAttributes(attrs, internal::R::styleable::GridLayout, defStyleAttr);

    setOrientation(ta&&ta->hasValue(internal::R::styleable::GridLayout_orientation) ? ta->getInt(internal::R::styleable::GridLayout_orientation,DEFAULT_ORIENTATION) : attrs.getInt("orientation",std::unordered_map<std::string,int>{
        {"horizontal",(int)HORIZONTAL},
        {"vertical",(int)VERTICAL}//
    },DEFAULT_ORIENTATION));
    setRowCount(ta&&ta->hasValue(internal::R::styleable::GridLayout_rowCount) ? ta->getInt(internal::R::styleable::GridLayout_rowCount, DEFAULT_COUNT) : attrs.getInt("rowCount", DEFAULT_COUNT));
    setColumnCount(ta&&ta->hasValue(internal::R::styleable::GridLayout_columnCount) ? ta->getInt(internal::R::styleable::GridLayout_columnCount, DEFAULT_COUNT) : attrs.getInt("columnCount", DEFAULT_COUNT));
    setUseDefaultMargins(ta&&ta->hasValue(internal::R::styleable::GridLayout_useDefaultMargins) ? ta->getBoolean(internal::R::styleable::GridLayout_useDefaultMargins, DEFAULT_USE_DEFAULT_MARGINS) : attrs.getBoolean("useDefaultMargins", DEFAULT_USE_DEFAULT_MARGINS));
    setAlignmentMode(ta&&ta->hasValue(internal::R::styleable::GridLayout_alignmentMode) ? ta->getInt(internal::R::styleable::GridLayout_alignmentMode,DEFAULT_ALIGNMENT_MODE) : attrs.getInt("alignmentMode",std::unordered_map<std::string,int>{
        {"alignBounds",(int)ALIGN_BOUNDS},
        {"alignMargins",(int)ALIGN_MARGINS}//
    },DEFAULT_ALIGNMENT_MODE));
    setRowOrderPreserved(ta&&ta->hasValue(internal::R::styleable::GridLayout_rowOrderPreserved) ? ta->getBoolean(internal::R::styleable::GridLayout_rowOrderPreserved, DEFAULT_ORDER_PRESERVED) : attrs.getBoolean("rowOrderPreserved", DEFAULT_ORDER_PRESERVED));
    setColumnOrderPreserved(ta&&ta->hasValue(internal::R::styleable::GridLayout_columnOrderPreserved) ? ta->getBoolean(internal::R::styleable::GridLayout_columnOrderPreserved, DEFAULT_ORDER_PRESERVED) : attrs.getBoolean("columnOrderPreserved", DEFAULT_ORDER_PRESERVED));
}
...
int GridLayout::getMargin(View* view, bool horizontal, bool leading){
    if (mAlignmentMode == ALIGN_MARGINS) {
        ...
        const LayoutParams* lp = getLayoutParams(view);
        const auto spec = horizontal ? lp->columnSpec : lp->rowSpec;
        const int index = leading ? spec->span.min : spec->span.max;
...
int GridLayout::clip(const Interval& minorRange, bool minorWasDefined, int count){
    ...
    const int min = minorWasDefined ? std::min(minorRange.min, count) : 0;
...
void GridLayout::validateLayoutParams() {
    ...
    for (int i = 0, N = getChildCount(); i < N; i++) {
        ...
        const auto majorSpec = horizontal ? lp->rowSpec : lp->columnSpec;
        ...
        const auto minorSpec = horizontal ? lp->columnSpec : lp->rowSpec;
...
int GridLayout::computeLayoutParamsHashCode() {
    ...
    for (int i = 0, N = getChildCount(); i < N; i++) {
        ...
        const LayoutParams* lp = (LayoutParams*) c->getLayoutParams();
    ...
    for (int i = 0, N = getChildCount(); i < N; i++) {
        ...
        if (firstPass) {
            ...
            if (spec->getAbsoluteAlignment(horizontal) == FILL) {
                ...
                const int cellSize = locations[span.max] - locations[span.min];
                const int viewSize = cellSize - getTotalMargin(c, horizontal);
                if (horizontal) {
...
void GridLayout::onLayout(bool changed, int left, int top, int w, int h){
    ...
    for (int i = 0, N = getChildCount(); i < N; i++) {
        View* c = getChildAt(i);
        if (c->getVisibility() == View::GONE) continue;
        const LayoutParams* lp = getLayoutParams(c);
        const auto columnSpec = lp->columnSpec;
        const auto rowSpec = lp->rowSpec;

        const Interval& colSpan = columnSpec->span;
        const Interval& rowSpan = rowSpec->span;
...
//--------------------------------------------------------------------------
const std::shared_ptr<GridLayout::Spec> GridLayout::Spec::UNDEFINED=GridLayout::spec(GridLayout::UNDEFINED);
...
std::shared_ptr<GridLayout::Spec> GridLayout::Spec::copyWriteSpan(const Interval& span)const{
    return std::make_shared<Spec>(startDefined, span, alignment, weight);
}

std::shared_ptr<GridLayout::Spec> GridLayout::Spec::copyWriteAlignment(const Alignment* alignment)const{
    return std::make_shared<Spec>(startDefined, span, alignment, weight);
}

int GridLayout::Spec::getFlexibility() const{
    return (alignment == GridLayout::UNDEFINED_ALIGNMENT && weight == 0) ? GridLayout::INFLEXIBLE : GridLayout::CAN_STRETCH;
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start, int size,const Alignment* alignment, float weight){
    return std::make_shared<Spec>(start != UNDEFINED, start, size, alignment, weight);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start,const Alignment* alignment, float weight){
    return spec(start, 1, alignment, weight);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start, int size,float weight){
    return spec(start, size, UNDEFINED_ALIGNMENT, weight);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start, float weight){
    return spec(start, 1, weight);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start, int size,const Alignment* alignment){
    return spec(start, size, alignment, Spec::DEFAULT_WEIGHT);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start,const Alignment* alignment){
    return spec(start, 1, alignment);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start, int size){
    return spec(start, size, UNDEFINED_ALIGNMENT);
}

std::shared_ptr<GridLayout::Spec> GridLayout::spec(int start){
    return spec(start, 1);
}
...
std::vector<GridLayout::Arc> GridLayout::Axis::topologicalSort(std::vector<GridLayout::Arc>& arcs){
    ...
    visited.resize(getCount()+1,(int)NEW);
    std::function<void(std::vector<int>&, std::vector<std::vector<GridLayout::Arc>>&, std::vector<GridLayout::Arc>&, int&, int)> walk;
    walk = [&walk](std::vector<int>&visited,std::vector<std::vector<GridLayout::Arc>>&arcsByVertex,
...
GridLayout::PackedMap<GridLayout::Interval,GridLayout::MutableInt>GridLayout::Axis::createLinks(bool min){
    ...
    std::vector<std::shared_ptr<Spec>>&keys = getGroupBounds().keys;
...
void GridLayout::Axis::solveAndDistributeSpace(std::vector<int>&a){
    ...
    while(deltaMin < deltaMax) {
        ...
        const int delta = (int) (((long) deltaMin + deltaMax) / 2);
        invalidateValues();
