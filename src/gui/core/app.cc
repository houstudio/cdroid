...
uint32_t App::arscGetIdentifier(const std::string& name, const std::string& type, const std::string& pkg) const {
...
//"@[package:][+]id/filname"
const std::string App::parseResource(const std::string&fullResId,std::string*res,std::string*ns)const {
...
ZIPArchive*App::getResource(const std::string&fullResId,std::string*relativeResID,std::string*outPackage)const {
...
ZIPArchive* App::findPakForPath(const std::string&package,const std::string&arscPath,
...
std::unique_ptr<std::istream> App::getInputStream(const std::string&fullresid,std::string*outpkg) {
...
Cairo::RefPtr<Cairo::ImageSurface> App::loadImage(std::istream&stream,int width,int height){
   return ImageDecoder::loadImage(stream,width,height); 
}

Cairo::RefPtr<Cairo::ImageSurface> App::loadImage(const std::string&resname,int width,int height){
...
Cairo::RefPtr<Cairo::ImageSurface> App::loadImage(int id,int width,int height){
