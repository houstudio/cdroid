...
static int64_t copticToJdn(int year, int month, int day) {
    return 1825029LL + 365LL * (year - 1) + (year - 1) / 4 + 30LL * (month - 1) + (day - 1);
}

static void jdnToCoptic(int64_t jdn, int& year, int& month, int& day) {
    int64_t r = jdn - 1825029LL;
    year = static_cast<int>((4 * r + 1463) / 1461);
    int64_t yearStart = copticToJdn(year, 1, 1);
    month = static_cast<int>((r - (yearStart - 1825029LL)) / 30) + 1;
    if (month > 13) month = 13;
    int64_t monthStart = copticToJdn(year, month, 1);
...
CopticCalendar::CopticCalendar() : GregorianCalendar() {
}

CopticCalendar::CopticCalendar(int year, int month, int date)
        : GregorianCalendar(year, month, date) {
}

CopticCalendar::CopticCalendar(int year, int month, int date, int hourOfDay, int minute, int second)
        : GregorianCalendar(year, month, date, hourOfDay, minute, second) {
}

void CopticCalendar::computeTime() {
    ...
    int64_t jdn = copticToJdn(year, month, day);
...
void CopticCalendar::computeFields() {
    ...
    int year, month, day;
    jdnToCoptic(jdn, year, month, day);
