...
int SidePropagation::distance(ViewGroup* sceneRoot, int viewX, int viewY, int epicenterX, int epicenterY,
    ...
    switch (side) {
    case Gravity::LEFT:
        distance = right - viewX + std::abs(epicenterY - viewY);
        break;
    case Gravity::TOP:
        distance = bottom - viewY + std::abs(epicenterX - viewX);
        break;
    case Gravity::RIGHT:
        distance = viewX - left + std::abs(epicenterY - viewY);
        break;
    case Gravity::BOTTOM:
        distance = viewY - top + std::abs(epicenterX - viewX);
        break;
    default:
        break;
    }
