...
Animator* Fade::createAnimation(View* view, float startAlpha, float endAlpha){
    ...
    if (DBG){ LOGD("%s: Created animator %p", LOG_TAG, (void*)anim); }

    FadeAnimatorListener* listenerState = new FadeAnimatorListener(view);
    Animator::AnimatorListener listener;
    listener.onAnimationStart = [listenerState](Animator& a, bool){ listenerState->onAnimationStart(a); };
    listener.onAnimationEnd   = [listenerState](Animator& a, bool){ listenerState->onAnimationEnd(a); delete listenerState; };
