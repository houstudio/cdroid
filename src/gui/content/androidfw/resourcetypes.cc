...
status_t ResChunk_header::validate_chunk(const ResChunk_header* chunk, size_t minSize,
...
status_t ResStringPool::setTo(const void* data, size_t size, bool copyData) {
    ...
    if (ResChunk_header::validate_chunk(chunk_header, sizeof(ResStringPool_header),
...
status_t ResXMLTree::setTo(const void* data, size_t size, bool copyData) {
    ...
    while (((const uint8_t*)chunk) < (mDataEnd - sizeof(ResChunk_header)) &&
        ...
        status_t err = ResChunk_header::validate_chunk(chunk, sizeof(ResChunk_header), mDataEnd, "XML");
        if (err != NO_ERROR) {
            mError = err;
            goto done;
        }
...
status_t ResXMLTree::validateNode(const ResXMLTree_node* node) const {
    ...
    status_t err = ResChunk_header::validate_chunk(&node->header, sizeof(ResXMLTree_node), mDataEnd,
                                  "ResXMLTree_node");
