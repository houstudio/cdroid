// Port of AOSP androidfw/AttributeFinder.h (frameworks/base/libs/androidfw/
// include/androidfw/AttributeFinder.h), namespace cdroid.
//
// KeyedVector (utils/KeyedVector.h) has no C++14 port in this tree — the
// shared-library package-offset map collapses to a std::map (same ordered
// key -> value semantics, indexOfKey = find).
//
// Copyright (C) 2016 The Android Open Source Project
// Licensed under the Apache License, Version 2.0.
#ifndef __CDROID_ANDROIDFW_ATTRIBUTE_FINDER_H__
#define __CDROID_ANDROIDFW_ATTRIBUTE_FINDER_H__
...
class BackTrackingAttributeFinder {
 ...
 private:
  ...
  std::map<uint32_t, Iterator> package_offsets_;
};
...
void BackTrackingAttributeFinder<Derived, Iterator>::JumpToClosestAttribute(
  ...
  switch (package_id) {
    ...
    case 0x7fu:
      ...
      break;
    default: {
      auto it = package_offsets_.find(package_id);
      if (it != package_offsets_.end()) {
...
void BackTrackingAttributeFinder<Derived, Iterator>::MarkCurrentPackageId(
  ...
  switch (package_id) {
    ...
    case 0x7fu:
      ...
      break;
    default:
      package_offsets_[package_id] = current_;
      break;
  }
}
...
Iterator BackTrackingAttributeFinder<Derived, Iterator>::Find(uint32_t attr) {
  ...
  } else if (current_ == end_ || attr < current_attr_) {
    // CDROID extension of the documented back-tracking: AOSP assumes the
    // REQUESTED attrs are id-sorted like the haystack (aapt2 emits R.java
    // styleables that way), so a same-package needle behind — or past the end
    // of — the cursor never needs rewinding. CDROID's hand-written internal
    // framework R styleables are NOT sorted, so rewind to the package's first
    // attribute here instead of missing the id.
    JumpToClosestAttribute(needle_package_id);
  }
...
#endif  // __CDROID_ANDROIDFW_ATTRIBUTE_FINDER_H__
