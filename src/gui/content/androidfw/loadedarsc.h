// Port of AOSP androidfw/LoadedArsc.h (frameworks/base/libs/androidfw/include/
// androidfw/LoadedArsc.h), namespace cdroid.
//
// Adaptations (the campaign's trim boundary):
//   * LoadedIdmap / OverlayStringPool are dropped (no RRO/idmap2 on CDROID —
//     overlays are build-time); the loaded_idmap parameters are gone.
//   * incfs::map_ptr/verified_map_ptr come from mapptr.h (the AOSP non-incfs
//     behavior: pointer + null-verification).
//   * base::expected from expected.h (the C++14 shim: own fixed-layout
//     base::optional, no optional-lite dependency — its layout flips with
//     -std, which breaks mixed-standard links).
//   * ResStringPool keeps this tree's raw-pointer faces (stringAt/string8At
//     with out-len); the AOSP expected<> faces are adapted at call sites.
//
// Copyright (C) 2016 The Android Open Source Project
// Licensed under the Apache License, Version 2.0.
#ifndef __CDROID_ANDROIDFW_LOADEDARSC_H__
#define __CDROID_ANDROIDFW_LOADEDARSC_H__
...
#include <content/androidfw/bytebucketarray.h>
...
struct TypeSpec {
    ...
    uint32_t GetFlagsForEntryIndex(uint16_t entry_index) const {
...
public:
    ...
    // the underlying ResStringPool API expects this. For now this is acceptable, but since the
    // default policy in AAPT2 is to build UTF-8 string pools, this needs to change.
    // Returns a partial resource ID, with the package ID left as 0x00. The caller is responsible
    // for patching the correct package ID to the resource ID.
    base::expected<uint32_t, androidfw::NullOrIOError> FindEntryByName(const std::u16string& type_name,
                                                                      const std::u16string& entry_name) const;

    static base::expected<incfs::map_ptr<ResTable_entry>, androidfw::NullOrIOError> GetEntry(
            incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index);

    static base::expected<uint32_t, androidfw::NullOrIOError> GetEntryOffset(
            incfs::verified_map_ptr<ResTable_type> type_chunk, uint16_t entry_index);

    static base::expected<incfs::map_ptr<ResTable_entry>, androidfw::NullOrIOError> GetEntryFromOffset(
    ...
    // the mipmap type.
    base::expected<base::monostate, androidfw::IOError> CollectConfigurations(
...
private:
    LoadedArsc(const LoadedArsc&) = delete;
    LoadedArsc& operator=(const LoadedArsc&) = delete;

    LoadedArsc() = default;
    bool LoadTable(const Chunk& chunk, package_property_t property_flags);

    std::unique_ptr<ResStringPool> global_string_pool_{new ResStringPool()};
    std::vector<std::unique_ptr<const LoadedPackage>> packages_;
};

}  // namespace cdroid

#endif  // __CDROID_ANDROIDFW_LOADEDARSC_H__
