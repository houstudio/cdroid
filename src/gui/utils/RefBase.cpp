...
void RefBase::incStrong(const void* id) const{
    ...
    LOGE_IF(c > 0, "incStrong() called on %p after last strong ref", refs);
#if PRINT_REFS
...
#endif
    LOGE_IF(BAD_STRONG(c), "decStrong() called on %p too many times",refs);
    if (c == 1) {
...
void RefBase::weakref_type::decWeak(const void* id){
    ...
    LOGE_IF(BAD_WEAK(c), "decWeak called on %p too many times",this);
    if (c != 1) return;
...
bool RefBase::weakref_type::attemptIncStrong(const void* id){
    ...
    LOGE_IF(curCount >= 0, "attemptIncStrong called on %p after underflow", this);
...
bool RefBase::weakref_type::attemptIncWeak(const void* id){
    ...
    LOGE_IF(curCount >= 0, "attemptIncWeak called on %p after underflow", this);
    while (curCount > 0) {
