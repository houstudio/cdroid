...
_svg_str_parse_angle (const char *str, double *angle_degrees)
{
    ...
    if (strlen (unit_str) == 0 || strcmp (unit_str, "deg") == 0)
        *angle_degrees = value;
    else if (strcmp (unit_str, "grad") == 0)
        *angle_degrees = value * 400.0 / 360.0;
    else if (strcmp (unit_str, "rad") == 0)
        *angle_degrees = value * 180.0 / M_PI;
    else
