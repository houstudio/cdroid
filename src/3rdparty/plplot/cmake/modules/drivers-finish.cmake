  ...
  if(PLD_${DEVICE})
    ...
    if(APPEND_DRIVER)
      if(ENABLE_DYNDRIVERS)
      	set(${DRIVER}_INFO)
	if(EXISTS ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.driver_info.in)
          file(STRINGS
  	    ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.driver_info.in
	    ${DRIVER}_INFO
	    )
	endif(EXISTS ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.driver_info.in)
	...
	if(OLD_WXWIDGETS)
          set(${DRIVER}_SOURCE
            ${PROJECT_SOURCE_DIR}/drivers/deprecated_${DRIVER}.cpp
            ${PROJECT_SOURCE_DIR}/drivers/deprecated_${DRIVER}_app.cpp
            ${PROJECT_SOURCE_DIR}/drivers/deprecated_${DRIVER}_dc.cpp
            ${PROJECT_SOURCE_DIR}/drivers/deprecated_${DRIVER}_gc.cpp
            ${${DRIVER}_SOURCE}
	    )
          if(HAVE_AGG)
            set(${DRIVER}_SOURCE
              ${PROJECT_SOURCE_DIR}/drivers/deprecated_${DRIVER}_agg.cpp
              ${${DRIVER}_SOURCE}
              )
          endif(HAVE_AGG)
	else(OLD_WXWIDGETS)
          set(${DRIVER}_SOURCE
            ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.cpp
	    ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}_comms.cpp
            ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}_dev.cpp
            ${${DRIVER}_SOURCE}
	    )
	endif(OLD_WXWIDGETS)
      elseif(DRIVER STREQUAL "psttf")
        set(${DRIVER}_SOURCE
	        ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.cc
	        ${${DRIVER}_SOURCE}
	        )
	  elseif(DRIVER STREQUAL "qt")
        ...
        set(${DRIVER}_SOURCE
	      ${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.c
	      ${${DRIVER}_SOURCE}
	      )
  ...
  if(${DRIVER}_INFO)
	...
	if(DEVICE STREQUAL "${DEVICE_INFO_NAME}")
      ...
      if(DEVICE_INFO_MATCHED)
        ...
        message(FATAL_ERROR "${PROJECT_SOURCE_DIR}/drivers/${DRIVER}.driver_info.in not consistent with ${PROJECT_SOURCE_DIR}/cmake/modules/drivers-init.cmake")
...
foreach(DRIVERS_DEVICE ${DRIVERS_DEVICE_LIST})
  string(REGEX REPLACE "^.*:(.*):.*:.*:.*$" "\\1" DRIVER ${DRIVERS_DEVICE})
  if(${DRIVER}_INFO)
    file(WRITE ${PROJECT_BINARY_DIR}/drivers/${DRIVER}.driver_info "")
    foreach(DEVICE_INFO ${${DRIVER}_INFO})
      string(REGEX REPLACE "^(.*:.*:.*:)(.*:)(.*:.*)$" "\\1${WRITEABLE_TARGET}\\2\\3" MODIFIED_DEVICE_INFO "${DEVICE_INFO}")
      file(APPEND ${PROJECT_BINARY_DIR}/drivers/${DRIVER}.driver_info "${MODIFIED_DEVICE_INFO}\n")
    endforeach(DEVICE_INFO ${${DRIVER}_INFO})
  endif(${DRIVER}_INFO)
