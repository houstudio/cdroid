...
extern "C" {
  ...
  void im_set_max_lens(void*hime,size_t max_sps_len, size_t max_hzs_len) {
    ((PINYINIME*)hime)->matrix_search->set_max_lens(max_sps_len, max_hzs_len);
  }
  ...
  void im_reset_search(void*hime) {
    ((PINYINIME*)hime)->matrix_search->reset_search();
  }
  ...
  const char* im_get_sps_str(void*hime,size_t *decoded_len) {
    return ((PINYINIME*)hime)->matrix_search->get_pystr(decoded_len);
  }

  char16* im_get_candidate(void*hime,size_t cand_id, char16* cand_str,
                        size_t max_len) {
    return ((PINYINIME*)hime)->matrix_search->get_candidate(cand_id, cand_str, max_len);
  }

  size_t im_get_spl_start_pos(void*hime,const uint16 *&spl_start) {
    return ((PINYINIME*)hime)->matrix_search->get_spl_start(spl_start);
  }

  size_t im_choose(void*hime,size_t choice_id) {
    return ((PINYINIME*)hime)->matrix_search->choose(choice_id);
  }

  size_t im_cancel_last_choice(void*hime) {
    return ((PINYINIME*)hime)->matrix_search->cancel_last_choice();
  }

  size_t im_get_fixed_len(void*hime) {
    return ((PINYINIME*)hime)->matrix_search->get_fixedlen();
  }
