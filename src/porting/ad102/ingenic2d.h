...
struct ingenic_2d_frame {
    int width;                                      /*用户申请的宽*/
    int height;                                     /*用户申请的高*/
    int align_width;                                /*2d模块实际操作的宽，需要按4像素点对齐*/
    int align_height;                               /*2d模块实际操作的高，需要按4像素点对齐*/
    int stride;                                     /*一行大小（字节）*/
...
/**
 * @brief 是否使能在使用时刷新源的cache，使能后会增加耗时，默认使能
 * @param ingenic_2d        2d设备句柄
 * @param enable            非0:使能，0:失能
 */
void ingenic_2d_enable_src_flush_cache(struct ingenic_2d *ingenic_2d, int enable);


/**
 * @brief 是否使能在使用时刷新目标的cache，使能后会增加耗时，默认使能
 * @param ingenic_2d        2d设备句柄
 * @param enable            非0:使能，0:失能
 */
void ingenic_2d_enable_dst_flush_cache(struct ingenic_2d *ingenic_2d, int enable);
