...
/*
 * @brief 删除已保存的网络配置
 * @param network_config_path wifi配置保存的路径，删除后的内容覆盖原文件
 * @param network_ssid wifi名称
 * @return 成功返回0,失败返回负数
*/
int wifi_remove_network_config(const char *network_config_path, const char *network_ssid);

/*
 * @brief 获取网络配置保存的信息
 * @param network_config_path wifi配置保存的路径
 * @param save_info 储存返回的信息
 * @param info_count save_info的大小
 * @return 成功返回实际个数,失败返回负数
*/
int wifi_get_save_info(const char *network_config_path, struct wifi_network_save_info *save_info, int info_count);
