...
struct jpegd_decoder_config {
    ...
    unsigned long phy_mem;   // 待解码的源数据物理地址(直接作为输入源,内存占用更少.需4k对齐)
    int uncropped;           // 默认会对非16对齐的宽高进行偏移裁切，置1可不裁
};
...
/**
 * @brief 设置phy_mem为解码输出地址,内存占用更少(需要4k对齐. 当mem, phy_mem, output_size都为0时,切换为自动分配output buff)
 * @param decoder decoder设备句柄
 * @param mem          输出源的虚拟地址
 * @param phy_mem      输出源的物理地址
 * @param output_size  输出源的buff大小
 * @return 成功返回0,失败返回-1
 */
int jpegd_decoder_set_output_mem(struct jpegd_decoder *decoder, struct jpegd_decoder_config *config, void *mem, unsigned long phy_mem, unsigned int output_size);
