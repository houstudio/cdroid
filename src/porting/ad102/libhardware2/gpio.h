...
/**
 * @brief 获得gpio的电平跳变的速度
 * @param fd gpio设备句柄,由gpio_open()函数获得
 * @param gpio 描述对应的gpio引脚，如"PB08"
 * @return 成功返回速度大小,失败返回负数
 */
int gpio_get_slew_rate(int fd, const char *gpio);

/**
 * @brief 设置gpio的schmitt触发特性
 * @param fd gpio设备句柄,由gpio_open()函数获得
 * @param gpio 描述对应的gpio引脚，如"PB08"
 * @param value 寄存器值，见手册
 * @return 成功返回0,失败返回负数
 */
int gpio_set_schmitt(int fd, const char *gpio, int value);

/**
 * @brief 获得gpio的schmitt触发特性
 * @param fd gpio设备句柄,由gpio_open()函数获得
 * @param gpio 描述对应的gpio引脚，如"PB08"
 * @return 成功返回寄存器值,失败返回负数
 */
