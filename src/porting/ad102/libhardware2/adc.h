...
struct adc_seq0_config {
    ...
    long is_enable_irq;               /* 是否使能中断，开启后保存每次采样的数据，可供读取，通常都需要使能 */
};
...
struct adc_seq1_config {
    ...
    long is_enable_irq;     /* 是否使能中断，开启后保存每次采样的数据，
                                可供读取，除了awd 和 dma 模式外，通常都需要使能 */
    int dma_mode;         //使用 dma 模式
};
...
/**
 * @brief 使能 ADC 的 seq1 采集序列, 仅适用与 x2600 和 ad10x
...
/**
 * @brief 失能 ADC 的 seq1 采集序列, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param adc_seq1 用户可配置的相关参数
 * @return 返回非负数表示成功，失败返回负数
 */
...
/**
 * @brief 读取 ADC 的 seq1 采集序列获取到的数据, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param timeout 读取超时时间
 * @param data 需要传入一个类似data[16]的数组，用户输出读取结果
 * @return 返回实际读取到的个数，对应采集序列输入的个数，失败返回负数
 */
int adc_seq1_read_timeout(long handle, int timeout, unsigned int *data);

/**
 * @brief 使能 ADC 的 seq2 采集序列, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param adc_seq1 用户可配置的相关参数
 * @return 返回非负数表示成功，失败返回负数
 */
int adc_seq2_enable(long handle, struct adc_seq2_config adc_seq2);

/**
 * @brief 失能 ADC 的 seq2 采集序列, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param adc_seq1 用户可配置的相关参数
 * @return 返回非负数表示成功，失败返回负数
 */
int adc_seq2_disable(long handle, struct adc_seq2_config adc_seq2);

/**
 * @brief 读取 ADC 的 seq2 采集序列获取到的数据, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param timeout 读取超时时间
 * @param data 需要传入一个类似data[16]的数组，用户输出读取结果
 * @return 返回实际读取到的个数，对应采集序列输入的个数，失败返回负数
 */
int adc_seq2_read_timeout(long handle, int timeout, unsigned short *data);

/**
 * @brief 使能 ADC 对应通道的 awd 功能，本身不具备采样功能，通常建议使用seq1的连续采集模式触发检测, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
...
/**
 * @brief 失能 ADC 对应通道的 awd 功能, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @return 返回非负数表示成功，失败返回负数
 */
...
/**
 * @brief 读取 ADC 的 awd 被触发过的通道有哪些, 仅适用与 x2600 和 ad10x
 * @param handle 句柄，由adc_open()获取
 * @param timeout 读取超时时间
