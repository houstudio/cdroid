...
int GFXInit() {
    ...
    void* preallocedVMem;
    ret = MI_SYS_MMA_Alloc(0,"mma_heap_name0",allocedSize,&preallocedMem);
...
int32_t GFXFillRect(GFXHANDLE surface,const GFXRect*rect,uint32_t color) {
    ...
    if(ngs->kbuffer) {
        ...
        ret=MI_GFX_QuickFill(0,&gfxsurf,&mirec,color,&fence);
...
int32_t GFXCreateSurface(int dispid,GFXHANDLE*surface,uint32_t width,uint32_t height,int32_t format,bool hwsurface) {
    ...
    if(surf->kbuffer&&(surf->hasLogo==0)) MI_SYS_MemsetPa(0,surf->kbuffer,0xFF000000,surf->msize);
...
int32_t GFXBlit(GFXHANDLE dstsurface,int dx,int dy,GFXHANDLE srcsurface,const GFXRect*srcrect) {
    ...
    if(nsrc->kbuffer && ndst->kbuffer){
        ret = MI_GFX_BitBlit(0,&gfxsrc,&stSrcRect,&gfxdst, &stDstRect,&opt,&fence);
        MI_GFX_WaitAllDone(0,TRUE,fence);
    }else{
