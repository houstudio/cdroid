...
typedef MI_S32  MI_VENC_DEV;
typedef MI_S32  MI_VENC_CHN;
typedef MI_S32  MI_VDEC_DEV;
...
typedef MI_U64  MI_HANDLE;
typedef MI_S32  MI_PCIE_DEV;
typedef MI_S32  MI_PCIE_CHN;
typedef MI_S32  MI_HVP_DEV;
typedef MI_S32  MI_HVP_CHN;
//-------------------------------------------------------------------------------------------------
//  Macros
//-------------------------------------------------------------------------------------------------
#define INIT_ST(st)                         \
...
#define MI_ConvertPtrArrayToPtr64Aarray(ptr64, ptr, num)                       \
    ({                                                                         \
        MI_S32 __ret = 0;                                                      \
        if(sizeof(MI_PTR) != sizeof(MI_PTR64))                                 \
        {                                                                      \
            MI_U32 __num = num;                                                \
            if (__num > 0 && __num <= (sizeof(ptr64)/sizeof(ptr64[0]))         \
                && __num <= (sizeof(ptr)/sizeof(ptr[0]))                       \
                && TYPECHECK(MI_PTR64, ptr64[0]) && TYPECHECK(MI_PTR, ptr[0])) \
            {                                                                  \
                MI_S32 i = 0;                                                  \
                for (i = __num - 1; i >= 0; i--)                               \
                {                                                              \
                    ptr64[i] = (MI_PTR64)(MI_VIRT)ptr[i];                      \
                }                                                              \
            }                                                                  \
            else                                                               \
            {                                                                  \
...
#define MI_ConvertPtr64ArrayToPtrAarray(ptr, ptr64, num)                       \
    ({                                                                         \
        MI_S32 __ret = 0;                                                      \
        if(sizeof(MI_PTR) != sizeof(MI_PTR64))                                 \
        {                                                                      \
            MI_U32 __num = num;                                                \
            if (__num > 0 && __num <= (sizeof(ptr)/sizeof(ptr[0]))             \
                && __num <= (sizeof(ptr64)/sizeof(ptr64[0]))                   \
                && TYPECHECK(MI_PTR, ptr[0]) && TYPECHECK(MI_PTR64, ptr64[0])) \
            {                                                                  \
                MI_U32 i = 0;                                                  \
                for (i = 0; i <= __num - 1; i++)                               \
                {                                                              \
                    ptr[i] = (MI_PTR)(MI_VIRT)ptr64[i];                        \
                }                                                              \
            }                                                                  \
            else                                                               \
            {                                                                  \
...
#define MI_ERR_ID (0x80000000 + 0x20000000)
...
#define MI_PCIE_INITIAL_ERROR_CODE (0x200)
...
#define MI_HVP_INITIAL_ERROR_CODE (0x320)
#define MI_IQSERVER_INITIAL_ERROR_CODE (0x340)
...
#define MI_DEF_ERR(module, level, errid) ((MI_S32)((MI_ERR_ID) | ((module) << 16) | ((level) << 12) | (errid)))
...
#ifndef __KERNEL__
__attribute__((weak)) char sgs_mi_so_version_string[] = MACRO_TO_STRING(SGS_MODULE_VERSION);
#endif
