#include <sys/ioctl.h>
#include <errno.h>
#include "include/ion_alloc.h"
#include "include/ion_alloc_list.h"
#include "include/ion_mem_alloc.h"
...
void* sunxi_ion_alloc_get_viraddr_byFd(int nShareFd)
{
    ...
    if (0 == flag)
    {
        loge("ion_alloc_phy2vir failed, do not find nShareFd %d \n", nShareFd);
    }
...
#else
    ret = ioctl(g_ion_alloc_context->cedar_fd, AW_MEM_FLUSH_CACHE_RANGE, &range);
#endif
    if (ret)
