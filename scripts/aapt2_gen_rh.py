...
def main():
    ...
    by_type = {}  # public ctype -> {cident_key: hex_id}
    private_attrs = {}  # ^attr-private name -> hex_id (-> cdroid::internal::R)
    for line in r.stdout.splitlines():
        ...
        if m and '$' not in m.group(3):
            ...
            ck = cident(rkey)
            if rtype == '^attr-private':
                # Private framework attrs (0x010dxxxx) go to a separate
                # cdroid::internal::R file (the com.android.internal.R equivalent),
                # not the public cdroid::R — like Android keeps internal.R out of
                # android.jar.
                if ck in private_attrs and private_attrs[ck] != rid:
                    sys.stderr.write("WARNING: internal R id collision: %s (0x%s vs 0x%s)\n"
                                     % (ck, private_attrs[ck], rid))
                private_attrs[ck] = rid
            else:
                tns = ctype(rtype)
    ...
    # Private framework attrs -> internal_R.h (cdroid::internal::R), the
    # com.android.internal.R equivalent — separate file, never mixed into the
    # public cdroid::R. Only framework-internal code includes this.
    internal_out = os.path.join(os.path.dirname(os.path.abspath(args.output)), 'internal_R.h')
    with open(internal_out, 'w') as f:
        f.write('#pragma once\n\n/*Generated by aapt2_gen_rh.py: private framework attrs')
        f.write(' (com.android.internal.R equivalent). Do not edit*/\n\n')
        f.write('namespace %s{\n\n' % args.namespace)
        f.write('namespace internal{\n')
        f.write('namespace R{\n')
        f.write('    namespace attr{\n')
        f.write('        enum : int {\n')
        for key, rid in sorted(private_attrs.items()):
            f.write('            %s = %s,\n' % (key, rid))
        f.write('        };\n')
        f.write('    }/*namespace attr*/\n\n')
        f.write('};//endof namespace R\n\n')
        ...
        f.write('}//endof namespace\n')
    sys.stderr.write('aapt2_gen_rh: %d public types/%d resources -> %s; %d private attrs -> %s\n'
                     % (len(by_type), sum(len(v) for v in by_type.values()), args.output,
                        len(private_attrs), internal_out))
