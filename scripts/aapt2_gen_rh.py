...
#
# Flags:
#   --only-public true/false  true  = ONLY public resources, wrapped in <ns>::R
#                             false = ALL resources (public+private), wrapped in
#                             <ns>::internal::R (com.android.internal.R equivalent)
#                             Default: false (all resources).
#
# Framework (cdroid) build calls this TWICE via pakbuilder:
#   1. --only-public true  -o R.h           → cdroid::R (public only)
#   2. --only-public false -o internal_R.h  → cdroid::internal::R (ALL)
#
...
def main():
    ...
    ap.add_argument('--only-public', choices=['true', 'false'], default='false',
                    help='true = public only (::R); false = ALL (::internal::R). Default: false.')
    args = ap.parse_args()
    only_public = (args.only_public == 'true')
    internal = not only_public  # false → wrap in ::internal::R
    ...
    scope = 'internal::R' if internal else 'R'
