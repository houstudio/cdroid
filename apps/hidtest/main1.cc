...
void print_device(struct hid_device_info *cur_dev) {
	printf("Device Found %p\n  type: %04hx %04hx\n  path: %s\n  serial_number: %ls",cur_dev, cur_dev->vendor_id, cur_dev->product_id, cur_dev->path, cur_dev->serial_number);
	printf("\n");
...
int main(int argc, char* argv[])
{
	...
	handle = hid_open(devs->vendor_id,devs->product_id,nullptr);
	if (!handle) {
		printf("unable to open device %x:%x",devs->vendor_id,devs->product_id);
		hid_exit();
 		return 1;
	}
